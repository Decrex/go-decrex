...
#
# The cluster contains a bootnode, a decrx node and multiple swarm nodes, with
...
# start_bootnode starts a bootnode which is used to bootstrap the decrx and
# swarm nodes
start_bootnode() {
...
# start_geth_node starts a decrx node with --datadir pointing at <base-dir>/decrx
# and a single, unlocked account with password "decrx"
start_geth_node() {
  local dir="${base_dir}/decrx"
  mkdir -p "${dir}"

  local password="decrx"
  echo "${password}" > "${dir}/password"

  # create an account if necessary
  if [[ ! -e "${dir}/keystore" ]]; then
    info "creating decrx account"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get decrx account address"
  fi
  ...
  start_node "decrx" "${GETH_IP}" "$(which decrx)" ${args[@]}
}
...
create_account() {
  ...
  decrx --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
