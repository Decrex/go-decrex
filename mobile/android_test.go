...
func TestAndroid(t *testing.T) {
	...
	workspace, err := ioutil.TempDir("", "decrx-android-")
	if err != nil {
	...
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.ethereum", "github.com/decrx/go-decrx/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "decrx.aar"), "decrx.aar", os.ModePerm)
